...
const HUD = {
  ...
  draw(ctx, player, boss, midBoss, score, subtitle, subtitleTimer) {
    ...
    // Boss HP
    if (boss && boss.alive) {
      const bossName = boss instanceof NullBoss ? 'NULL' : 'LEVIATHAN';
      this._hpBar(ctx, Game.width / 2 - 368, 19, 736, 22, boss.hp, boss.maxHp, '#ee3333', bossName);
    }
    // Mid-boss HP
    if (midBoss && midBoss.alive) {
      let name = 'BOSS';
      if (midBoss.type === 'cart_shark') name = 'CART SHARK';
      else if (midBoss.type === 'signal_jelly') name = 'SIGNAL JELLY';
      else if (midBoss.type === 'abyssAngler') name = 'ABYSS ANGLER';
