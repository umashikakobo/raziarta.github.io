...
class PenguinBullet {
  ...
  update() {
    ...
    if(this.x<-30||this.x>990) this.alive=false;
  }
...
class PenguinBomb {
  ...
  update() {
    ...
    if(this.y+this.hh>=Physics.GROUND_Y||this.x<-30||this.x>990) this.explode();
  }
  explode() {
...
class CanBullet {
  ...
  update(){this.x+=this.vx;this.y+=this.vy;if(this.x<-20||this.x>980||this.y<-20||this.y>560)this.alive=false;}
...
class BossBullet {
  ...
  update(){this.timer++;this.x+=this.vx;this.y+=this.vy;if(this.type==='debris')this.vy+=0.15;if(this.x<-30||this.x>990||this.y>560||this.timer>300)this.alive=false;}
  draw(ctx){
