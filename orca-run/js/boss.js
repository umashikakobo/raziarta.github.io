...
class Boss {
  ...
  draw(ctx) {
    ...
    // Calculate organic body tilt and vertical offset based on state
    let tilt = 0;
    let yOff = 0;
    if (this.state === 'slam_windup') {
      tilt = -(this.armY / 70) * 0.15; // Tilt backwards to wind up
      yOff = -(this.armY / 70) * 20;   // Rise up slightly
    } else if (this.state === 'slam_hit') {
      tilt = 0.2; // Slam forward
      yOff = 15;  // Crash down
    } else if (this.state === 'beam_lock') {
      tilt = Math.sin(this.frame * 0.5) * 0.05 * this.beamCharge; // Vibrate while charging
    } else if (this.state === 'beam_fire') {
      tilt = -0.1 + Math.sin(this.frame * 0.8) * 0.03; // Recoil and heavy vibration
    }

    // Apply transformations to the whole body
    ...
    // Draw an organic looking tentacle instead of simple rectangles
    ctx.save();
    ctx.fillStyle = this.damageFlash > 0 ? '#cc5533' : '#3a4a5a';
    if (this.state === 'slam_windup' || this.state === 'slam_hit') {
       ctx.translate(-20, -50); // Shoulder joint
       // Rotate arm based on state
       const armRot = this.state === 'slam_windup' ? -(this.armY / 70) * 1.2 : 1.5;
       ctx.rotate(armRot);
       
       // Draw a thick tentacle
       ctx.beginPath();
       ctx.moveTo(-15, 0);
       ctx.quadraticCurveTo(-30, -60, -10, -120);
       ctx.quadraticCurveTo(10, -60, 15, 0);
       ctx.fill();
       
       // Draw some suckers
       ctx.fillStyle = '#cc8866';
       ctx.beginPath(); ctx.arc(-22, -40, 5, 0, Math.PI * 2); ctx.fill();
       ctx.beginPath(); ctx.arc(-18, -70, 4, 0, Math.PI * 2); ctx.fill();
       ctx.beginPath(); ctx.arc(-14, -100, 3, 0, Math.PI * 2); ctx.fill();
    }
    ctx.restore();

    // Draw glowing mouth energy
    if (this.beamCharging || this.beamFiring) {
      const mOff = (this.mouthY - this.y);
      const intensity = this.beamFiring ? 1 : this.beamCharge;
      ctx.fillStyle = `rgba(255,100,30,${intensity * 0.85})`;
      ctx.beginPath();
      const glowSize = this.beamFiring ? 25 + Math.sin(this.frame) * 5 : 15 + Math.sin(this.frame * 0.5) * 5;
      ctx.arc(-46, mOff, glowSize, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.beginPath();
      ctx.arc(-46, mOff, glowSize * 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
