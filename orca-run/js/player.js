...
class Player {
  ...
  update(keys, mouseL, mouseR, platforms, camX, screenW, screenH) {
    ...
    // Input processing
    const isWDown = keys['w'] || keys['W'] || keys['Shift'];
    ...
    if (jumpJustPressed && !this.crouching) {
      if (this.onGround) {
        ...
        this.vy = -11.0; // Strengthened from -7.0 to -11.0
        this.jumpCount++;
  ...
  shootNormal() {
    ...
    ProjectileManager.addPlayerBullet(bx, by, this.facing, 'normal', mul, this.evolved);
  ...
  shootBomb() {
    ...
    const mul = this.evolved ? this.atkMultiplier * 2 : this.atkMultiplier;
    ProjectileManager.addPlayerBullet(bx, by, this.facing, 'bomb', mul, this.evolved);
