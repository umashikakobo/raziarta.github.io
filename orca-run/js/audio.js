...
const AudioManager = {
  ...
  roadParts:{}, bossParts:{}, abyssParts:{}, lastBossParts:{}, sePool:{},
  ...
  async init() {
    ...
    const aPadChords = [['C2', 'G2', 'C3'], ['Ab1', 'Eb2', 'Ab2'], ['Bb1', 'F2', 'Bb2'], ['G1', 'D2', 'G2']];
    const aPadL = new Tone.Sequence((t, i) => { aPad.triggerAttackRelease(aPadChords[i], '4m', t, 0.4); }, [0, 1, 2, 3], '4m').start(0);
    
    const aLead = new Tone.Synth({oscillator: {type: 'sine'}, envelope: {attack: 0.5, decay: 2, sustain: 0.1, release: 3}, volume: 0}).connect(new Tone.FeedbackDelay('2n', 0.6).connect(rev));
    const aLeadN = ['G4', null, 'Bb4', null, 'C5', null, 'D5', null, 'G5', null, null, null, 'F5', null, 'D5', null];
    const aLeadL = new Tone.Sequence((t, n) => { if (n) aLead.triggerAttackRelease(n, '1m', t, 0.3); }, aLeadN, '2n').start(0);
    ...
    const aDrumL = new Tone.Sequence((t) => { aKick.triggerAttackRelease('C1', '8n', t, 0.5); }, ['k', null, null, null, null, null, null, null], '4n').start(0);
    ...
    const nLeadN = ['C4', 'C4', 'Eb4', 'C4', 'F4', 'C4', 'Gb4', 'F4', 'Eb4', 'C4', null, 'C3', 'C3', null, null, null];
    const nLeadL = new Tone.Sequence((t, n) => { if (n) nLead.triggerAttackRelease(n, '16n', t, 0.8); }, nLeadN, '16n').start(0);
    ...
    const nDrumL = new Tone.Sequence((t, v) => { if (v) nNoise.triggerAttackRelease('32n', t, 0.6); }, ['n', 'n', null, 'n', 'n', null, 'n', null], '16n').start(0);
  ...
  startRoadBGM(){if(!this.initialized||this.bgmPlaying==='road')return;this.stopAll();console.log("Playing Road BGM");Tone.Transport.bpm.value=108;this.roadParts.loops.forEach(l=>l.start(0));this.roadParts.noise.start();Tone.Transport.start();this.bgmPlaying='road';},
  startAbyssBGM(){if(!this.initialized||this.bgmPlaying==='abyss')return;this.stopAll();console.log("Playing Abyss BGM");Tone.Transport.bpm.value=84;this.abyssParts.loops.forEach(l=>l.start(0));Tone.Transport.start();this.bgmPlaying='abyss';},
  startBossBGM(){if(!this.initialized||this.bgmPlaying==='boss')return;this.stopAll();console.log("Playing Boss BGM");Tone.Transport.bpm.value=144;this.bossParts.loops.forEach(l=>l.start(0));Tone.Transport.start();this.bgmPlaying='boss';},
  startLastBossBGM(){if(!this.initialized||this.bgmPlaying==='lastboss')return;this.stopAll();console.log("Playing Last Boss BGM");Tone.Transport.bpm.value=156;this.lastBossParts.loops.forEach(l=>l.start(0));Tone.Transport.start();this.bgmPlaying='lastboss';},
  stopAll(){try{Tone.Transport.stop();this.roadParts.loops?.forEach(l=>l.stop());this.roadParts.noise?.stop();this.bossParts.loops?.forEach(l=>l.stop());this.abyssParts.loops?.forEach(l=>l.stop());this.lastBossParts.loops?.forEach(l=>l.stop());this.bgmPlaying=null;console.log("Stop BGM");}catch(e){}},
