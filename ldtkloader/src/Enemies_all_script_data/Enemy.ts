...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(
        ...
        this.setImmovable(true); // プレイヤーに押されて滑らないようにする
        if ((this as any).setPushable) {
            (this as any).setPushable(false);
        }

        // 敵の基本サイズを 32x32 に固定（LDtk側の見た目上のサイズが巨大化するのを防ぐため）
        this.setDisplaySize(32, 32);
        (this.body as Phaser.Physics.Arcade.Body).setSize(32, 32);
        this.setOrigin(0.5, 0.5); // スプライトの原点を中心に設定（山差防止）

        this.setMaxVelocity(Math.max(moveSpeed, 300), 600);
    }
