...
function packData(d) {
            switch (d.type) {
                case 'sync': return [PKT.SYNC, d.entities.map(e => [e.id, round2(e.pos.x), round2(e.pos.y), round2(e.pos.z), round2(e.vel.x), round2(e.vel.y), round2(e.vel.z), e.isAlive ? 1 : 0, e.color, e.name, e.team, e.kills || 0]), Date.now()]; // kills を追加
                case 'client_update': return [PKT.CLIENT_UPDATE, [round2(d.input.x), round2(d.input.z), d.input.jump ? 1 : 0], d.name]; // jump を追加
                case 'log': return [PKT.LOG, d.msg];
                case 'apply_settings': return [PKT.APPLY_SETTINGS, d.isSurvival ? 1 : 0, d.isKnockback ? 1 : 0, d.isEscalation ? 1 : 0, d.lives || 0, d.kbRate || 1, d.remoteTeams || {}, d.kbExp || 2, d.isOriginal ? 1 : 0, d.syncParams || {}, d.escStart || ESC_DEFAULT_START_SPEED, d.escMax || ESC_DEFAULT_MAX_SPEED, d.isSync ? 1 : 0];
                ...
                case 'reset_event': return [PKT.RESET, d.id, round2(d.pos.x), round2(d.pos.y), round2(d.pos.z), d.lives || 0, d.stress || 0, d.given || 0, d.kills || 0];
                ...
                case 'event':
                    ...
                    if (d.eventType === 'stat_change') {
                        const safeLives = (typeof d.lives === 'number') ? d.lives : -1; // 0は有効な残機のため、無効値は-1を送信
                        ...
                        const safeKills = (typeof d.kills === 'number') ? d.kills : 0;
                        return [PKT.STAT_CHANGE, d.id, safeLives, safeStress, safeGiven, safeKills];
                    }
                    if (d.eventType === 'param_update') return [PKT.PARAM_UPDATE, round2(d.maxSpeed), parseFloat(d.accel.toFixed(3))];
                    if (d.eventType === 'shockwave') return [PKT.SHOCKWAVE, round2(d.x), round2(d.y), round2(d.z), round2(d.nx), round2(d.ny), round2(d.nz), round2(d.speed)];
                    return d;
...
function unpackData(arr) {
            ...
            const type = arr[0];
            switch (type) {
                case PKT.SYNC: return { type: 'sync', entities: arr[1].map(e => ({ id: e[0], pos: { x: e[1], y: e[2], z: e[3] }, vel: { x: e[4], y: e[5], z: e[6] }, isAlive: !!e[7], color: e[8], name: e[9], team: e[10], kills: e[11] || 0 })), serverTime: arr[2] }; // kills を追加
                case PKT.CLIENT_UPDATE: return { type: 'client_update', input: { x: arr[1][0], z: arr[1][1], jump: !!arr[1][2] }, name: arr[2] }; // jump を追加
                case PKT.LOG: return { type: 'log', msg: arr[1] };
                case PKT.APPLY_SETTINGS: return { type: 'apply_settings', isSurvival: !!arr[1], isKnockback: !!arr[2], isEscalation: !!arr[3], lives: arr[4], kbRate: arr[5], remoteTeams: arr[6], kbExp: arr[7] || 2, isOriginal: !!arr[8], syncParams: arr[9] || null, escStart: arr[10] || ESC_DEFAULT_START_SPEED, escMax: arr[11] || ESC_DEFAULT_MAX_SPEED, isSync: !!arr[12] };
                ...
                case PKT.RESET: return { type: 'reset_event', id: arr[1], pos: { x: arr[2], y: arr[3], z: arr[4] }, lives: arr[5], stress: arr[6], given: arr[7] || 0, kills: arr[8] || 0 };
                ...
                case PKT.STAT_CHANGE: return { type: 'event', eventType: 'stat_change', id: arr[1], lives: arr[2], stress: arr[3], given: arr[4] || 0, kills: arr[5] || 0 };
                case PKT.PARAM_UPDATE: return { type: 'event', eventType: 'param_update', maxSpeed: arr[1], accel: arr[2] };
                case PKT.SHOCKWAVE: return { type: 'event', eventType: 'shockwave', x: arr[1], y: arr[2], z: arr[3], nx: arr[4], ny: arr[5], nz: arr[6], speed: arr[7] };
